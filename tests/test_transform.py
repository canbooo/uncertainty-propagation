...
from experiment_design import variable, orthogonal_sampling
...
class TestInverseTransformSampler:
    ...
    @staticmethod
    def get_instance(distributions: list[rv_frozen]) -> module_under_test.InverseTransformSampler:
        return module_under_test.InverseTransformSampler(
            variable.DesignSpace(variables=variable.create_variables_from_distributions(distributions))
        )
...
def test_solve_nataf_normal_dist(current_correlation_matrix: np.ndarray) -> None:
    ...
    correlate, uncorrelate = module_under_test.solve_nataf(marginals, current_correlation_matrix)
...
def test_solve_nataf_non_normal_dist(current_correlation_matrix: np.ndarray, distributions_to_test: list[rv_frozen]) -> None:
    for dist1, dist2 in combinations(distributions_to_test, 2):
        correlate, uncorrelate = module_under_test.solve_nataf([dist1, dist2],
                                                               current_correlation_matrix)
        full_uncorrelate = uncorrelate.dot(uncorrelate.T)
        full_correlate = correlate.T.dot(correlate)
        test_matrix = full_correlate.dot(full_uncorrelate)
        assert np.isclose(test_matrix, np.eye(2), rtol=0., atol=1e-5).all()
...
class TestNatafTransformation:
    ...
    @staticmethod
    def get_instance(distributions: list[rv_frozen], correlation_matrix: np.ndarray) -> module_under_test.InverseTransformSampler:
    ...
    def test_transform(self, current_correlation_matrix: np.ndarray, distributions_to_test: list[rv_frozen]) -> None:
        np.random.seed(42)
        for dist1, dist2 in combinations(distributions_to_test, 2):
            designer = orthogonal_sampling.OrthogonalSamplingDesigner(target_correlation=current_correlation_matrix)
            doe = designer.design(
            ...
            if not np.isclose(np.corrcoef(transformed, rowvar=False), np.eye(2), rtol=5e-2, atol=1e-1).all():
                assert np.isclose(np.corrcoef(transformed, rowvar=False), np.eye(2), rtol=5e-2, atol=1e-1).all()


    def test_inverse_transform(self, current_correlation_matrix: np.ndarray,
                               distributions_to_test: list[rv_frozen]) -> None:
        np.random.seed(42)
        for dist1, dist2 in combinations(distributions_to_test, 2):
            designer = orthogonal_sampling.OrthogonalSamplingDesigner(target_correlation=0)
            doe = designer.design(
                variables=variable.create_variables_from_distributions([stats.norm(0, 1) for _ in range(2)]),
                sample_size=1000,
                steps=10
            )
            instance = self.get_instance([dist1, dist2], current_correlation_matrix)
            untransformed = instance.inverse_transform(doe)
            assert np.isclose(np.corrcoef(untransformed, rowvar=False), current_correlation_matrix, rtol=5e-2, atol=5e-2).all()
